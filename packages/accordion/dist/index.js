...
var AccordionError =
/*#__PURE__*/
function (_SuperError) {
  _inherits(AccordionError, _SuperError);

  function AccordionError(message) {
    var _this;

    _classCallCheck(this, AccordionError);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(AccordionError).call(this, message));
    _this.name = '@stereorepo/accordion';
    return _this;
  }
...
/* harmony default export */ var Error = (AccordionError);
...
var ScrollToPlugin_doc = (gsap_TweenLite["f" /* _gsScope */].document || {}).documentElement,
		...
		_max = function(element, axis) {
			var dim = (axis === "x") ? "Width" : "Height",
				...
				client = "client" + dim,
				body = document.body;
			return (element === _window || element === ScrollToPlugin_doc || element === body) ? Math.max(ScrollToPlugin_doc[scroll], body[scroll]) - (_window["inner" + dim] || ScrollToPlugin_doc[client] || body[client]) : element[scroll] - element["offset" + dim];
		},
		...
		_buildGetter = function(e, axis) { //pass in an element and an axis ("x" or "y") and it'll return a getter function for the scroll position of that element (like scrollTop or scrollLeft, although if the element is the window, it'll use the pageXOffset/pageYOffset or the documentElement's scrollTop/scrollLeft or document.body's. Basically this streamlines things and makes a very fast getter across browsers.
			...
			if (e === _window) {
				...
				} else if (ScrollToPlugin_doc[p] != null) {
					e = ScrollToPlugin_doc;
				} else {
					e = document.body;
				}
			}
		...
		_getOffset = function(element, container) {
			var rect = _unwrapElement(element).getBoundingClientRect(),
				...
				cRect = isRoot ? {top:ScrollToPlugin_doc.clientTop - (window.pageYOffset || ScrollToPlugin_doc.scrollTop || b.scrollTop || 0), left:ScrollToPlugin_doc.clientLeft - (window.pageXOffset || ScrollToPlugin_doc.scrollLeft || b.scrollLeft || 0)} : container.getBoundingClientRect(),
		...
		ScrollToPlugin_parseVal = function(value, target, axis, currentVal) {
		...
		ScrollToPlugin = gsap_TweenLite["f" /* _gsScope */]._gsDefine.plugin({
			...
			init: function(target, value, tween) {
				...
				if (value.x != null) {
					this._addTween(this, "x", this.x, ScrollToPlugin_parseVal(value.x, target, "x", this.x) - (value.offsetX || 0), "scrollTo_x", true);
					this._overwriteProps.push("scrollTo_x");
				} else {
					this.skipX = true;
				}
				if (value.y != null) {
					this._addTween(this, "y", this.y, ScrollToPlugin_parseVal(value.y, target, "y", this.y) - (value.offsetY || 0), "scrollTo_y", true);
...
/* eslint-disable */
	...
	gsap_TweenLite["f" /* _gsScope */]._gsDefine("plugins.CSSPlugin", ["plugins.TweenPlugin","TweenLite"], function() {
		...
		var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
			...
			_forcePT = {},
			_dummyElement = {style:{}},
			_doc = gsap_TweenLite["f" /* _gsScope */].document || {createElement: function() {return _dummyElement;}},
				...
				if (rec) {
					...
					if (tm.svg) { //if we're supposed to apply transforms to the SVG element's "transform" attribute, make sure there aren't any CSS transforms applied or they'll override the attribute ones. Also clear the transform attribute if we're using CSS, just to be clean.
						if (_useSVGTransformAttr && t.style[_transformProp]) {
							gsap_TweenLite["g" /* default */].delayedCall(0.001, function(){ //if we apply this right away (before anything has rendered), we risk there being no transforms for a brief moment and it also interferes with adjusting the transformOrigin in a tween with immediateRender:true (it'd try reading the matrix and it wouldn't have the appropriate data in place because we just removed it).
...
/* eslint-disable */
		...
		var BezierPlugin_RAD2DEG = 180 / Math.PI,
			...
			_r3 = [],
			_corProps = {},
			BezierPlugin_globals = gsap_TweenLite["f" /* _gsScope */]._gsDefine.globals,
...
gsap_TweenLite["f" /* _gsScope */]._gsDefine("easing.Back", ["easing.Ease"], function() {
		...
		var w = (gsap_TweenLite["f" /* _gsScope */].GreenSockGlobals || gsap_TweenLite["f" /* _gsScope */]),
			...
			_easeReg = gsap_TweenLite["b" /* Ease */].register || function(){}, //put an empty function in place just as a safety measure in case someone loads an OLD version of TweenLite.js where Ease.register doesn't exist.
...
var Sine = gsap_TweenLite["h" /* globals */].Sine;
...
var ensureScrollTo = ScrollToPlugin;
...
function () {
  function Accordion(_ref) {
    var _ref$containerSelecto = _ref.containerSelector,
        containerSelector = _ref$containerSelecto === void 0 ? '.js-accordion' : _ref$containerSelecto,
        _ref$clickedSelector = _ref.clickedSelector,
        clickedSelector = _ref$clickedSelector === void 0 ? '.js-clicked-element' : _ref$clickedSelector,
        _ref$contentSelector = _ref.contentSelector,
        contentSelector = _ref$contentSelector === void 0 ? '.js-content-element' : _ref$contentSelector,
        _ref$contentWrapperSe = _ref.contentWrapperSelector,
        contentWrapperSelector = _ref$contentWrapperSe === void 0 ? '.js-content-element' : _ref$contentWrapperSe,
        _ref$offsetY = _ref.offsetY,
        offsetY = _ref$offsetY === void 0 ? 0 : _ref$offsetY,
        _ref$ease = _ref.ease,
        ease = _ref$ease === void 0 ? gsap_TweenLite["c" /* Power1 */].easeInOut : _ref$ease,
        _ref$scrollDelay = _ref.scrollDelay,
        scrollDelay = _ref$scrollDelay === void 0 ? 300 : _ref$scrollDelay,
  ...
  _createClass(Accordion, [{
    key: "clickHandler",
    value: function clickHandler(clickedElement) {
      var _this = this;

      var clickedElementParent = clickedElement.parentElement;
      ...
      var _query = Object(dist["query"])({
        ...
        ctx: clickedElementParent
          ...
          _query2 = _slicedToArray(_query, 1),
          contentWrapper = _query2[0];

      var alreadyActivated = clickedElementParent.classList.contains('activated');
      ...
      if (!contentWrapper && !this.silent) {
        throw new Error('No accordion content wrapper found, try changing the contentWrapperSelector value in the Accordion declaration');
      }

      if (!content && !this.silent) {
        throw new Error('No accordion content found, try changing the contentSelector value in the Accordion declaration');
      }

      var maxHeight = content.getBoundingClientRect().height;
      var sameLevelAccordions = Object(dist["query"])({
        selector: this.containerSelector,
        ctx: clickedElementParent.parentElement
      });

      if (contentWrapper.style.maxHeight === 'none') {
        TweenMax_TweenMax.set(contentWrapper, {
      ...
      Object(dist["forEach"])(sameLevelAccordions, function (resetParent) {
        resetParent.classList.remove('activated');
        TweenMax_TweenMax.to(Object(dist["query"])({
          selector: _this.contentWrapperSelector,
          ctx: resetParent
        }), 0.3, {
          maxHeight: 0,
          opacity: 0,
      ...
      TweenMax_TweenMax.to(contentWrapper, 0.3, {
        maxHeight: maxHeight,
        opacity: 1,
        ease: this.ease,
      ...
      if (!this.noScroll) {
        setTimeout(function () {
          var offset = clickedElement.getBoundingClientRect().top + window.scrollY;
          TweenMax_TweenMax.to(window, 0.5, {
            scrollTo: {
              y: offset,
    ...
    value: function initializeAccordions() {
      ...
      this.accordions = Object(dist["query"])({
        selector: this.containerSelector
      });

      if (!this.accordions.length && !this.silent) {
        throw new Error('No accordion found, try changing the containerSelector value in the Accordion declaration');
      }

      Object(dist["forEach"])(this.accordions, function (accordion) {
        var _query5 = Object(dist["query"])({
          selector: _this2.clickedSelector,
          ctx: accordion
        }),
            _query6 = _slicedToArray(_query5, 1),
            clickedElement = _query6[0];

        if (clickedElement) {
          var clickHandlerBuffer = function clickHandlerBuffer() {
            _this2.clickHandler(clickedElement);
          };

          clickedElement.addEventListener('click', clickHandlerBuffer, false);
          _this2.clickedElementsList = [].concat(_toConsumableArray(_this2.clickedElementsList), [{
            clickedElement: clickedElement,
            clickHandlerBuffer: clickHandlerBuffer
          }]);
        } else if (!_this2.silent) {
          throw new Error('No element to click on found, try changing the clickedSelector value in the Accordion declaration');
        }
      });
    }
  }, {
    key: "destroyAccordions",
    value: function destroyAccordions() {
      var _this3 = this;

      Object(dist["forEach"])(this.clickedElementsList, function (_ref2) {
        var clickedElement = _ref2.clickedElement,
            clickHandlerBuffer = _ref2.clickHandlerBuffer;
        var clickedElementParent = clickedElement.parentElement;

        var _query7 = Object(dist["query"])({
          selector: _this3.contentWrapperSelector,
          ctx: clickedElementParent
        }),
            _query8 = _slicedToArray(_query7, 1),
            contentWrapper = _query8[0];
...
// CONCATENATED MODULE: ./src/index.js
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "Accordion", function() { return src_Accordion; });

var src_Accordion = components_Accordion;
/* harmony default export */ var src = __webpack_exports__["default"] = ({
  Accordion: src_Accordion
});
